...
### Token listing
  ...
  There are four C macro definitions in the starter code `src/scanner.l`: **`token`**,　**`tokenChar`**, and **`tokenString`**. You **should** write your scanner actions using the aforementioned macros or any other ones you defined in this manner for implementing the **`token listing`** option.
...
+ [**Stringizing operator (#)** - C/C++ preprocessor operator for Visual Studio 2019 in MS docs ](https://docs.microsoft.com/zh-tw/cpp/preprocessor/stringizing-operator-hash?view=vs-2019)
  ...
  The first argument of all four macros is a string. This string names the token that will be passed to the parser.  The macro **`tokenString`** takes a second argument, which must be a string. Some examples are given below:
...
+ If **`T`** (token option) is on, each token should be printed on a separate line, surrounded by angle brackets.
    + The token **Keywords** should be printed as **`KW+token`**.
    + The token **Identifiers** should be printed as **`id: +token`**.
    + The token **Integer Constants** should be printed as **`integer: +token`** or **`oct_integer: +token`**.
    + The token **Floating-Point Constants** should be printed as **`float: +token`**.
    + The token **Scientific Notations** should be printed as **`scientific: +token`**.
    + The token **String Constants** should be printed as **`string: +token`**.
